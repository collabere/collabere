...
export default function HomePage(props) {
  const [shouldRedirectFlag, setShouldRedirectFlag] = React.useState(false);
  ...
  useEffect(() => {
    if (localStorage.getItem("token")) {
      setShouldRedirectFlag(true);
    }
  }, []);

  if (shouldRedirectFlag) {
    return <Redirect to={`/clients/${localStorage.getItem("username")}`} />;
  }
  ...
  return (
    <div style={backgroundStyle}>
      <React.Fragment>
        ...
        <Container>
          <Box my={2}>
            <InfluencerSection dark={true} id="section1" />
            <BrandSection id="section2" />
            <HowItWorksSection dark={false} id="section3" />
          </Box>
        </Container>
        <ScrollTop {...props}>
